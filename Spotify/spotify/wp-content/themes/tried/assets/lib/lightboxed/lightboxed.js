;( function ( $ , window , document , undefined ) {
	...
	$.extend( Lightboxed.prototype , {
		...
		init : function () {
			...
			this.Obj.each( function (i) { that.addToCache( this, i ); } );
		...
		addToCache : function ( element, index ) {
			...
			if ( this.Obj.length > 1 ) {
				let isShow = true;
				let numb = jQuery( element ).attr( 'data-numb' );
				if (numb && (index > parseInt(numb) || index < 1)) {
					isShow = false;
				}
				if (isShow) {
